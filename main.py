...
for item in contacts_list_clean:
    ...
    phone_pattern = r"(\+7|8)(\s*)(\(?)(\d{3})(\)?)(\s*)(\-*)(\d{3})(-*)(\d{2})(-*)(\d{2})(\s*)(\(?)(\s?(доб)?)(\.?)(\s*)(\d*)(\)?)"
...
for item in contacts_list_clean_search[1:]:
    j += 1
    k = 0
    for another_item in contacts_list_clean:

        # добавляем в новый список уникальные записи
        if contacts_list_clean_search[j][-1] not in contacts_list_clean[k][-1]:
            k += 1
            if k == len(contacts_list_clean):
                contacts_list_clean.append(item)
                break
